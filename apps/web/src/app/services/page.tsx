  ...
  return (
    <main className="bg-[#E8F4FF] text-[#1F2A24]">
          ...
          {pillars.map((p) => {
            ...
            return (
              <a
                key={p.id}
                href={`#${p.id}`}
                className="inline-flex items-center gap-2 rounded-full border border-[#B0CFEE] px-4 py-2 text-sm text-[#5B6157] transition-colors hover:border-[#6E7F63] hover:text-[#1F2A24]"
              >
      ...
      {pillars.map((pillar, idx) => {
        ...
        return (
          <section
            key={pillar.id}
            id={pillar.id}
            className={`scroll-mt-24 ${idx % 2 === 1 ? 'bg-white border-y border-[#B0CFEE]' : ''}`}
          >
            <div className="mx-auto max-w-7xl px-5 sm:px-8 py-24 md:py-28">
              <div className="grid grid-cols-1 items-center gap-14 lg:grid-cols-2">
                {/* Image */}
                <ScrollReveal className={reversed ? 'lg:order-2' : ''}>
                  <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-[#B0CFEE] bg-gradient-to-br from-[#6E7F63] to-[#4F5E46]">
                ...
                <ScrollReveal delay={100} className={reversed ? 'lg:order-1' : ''}>
                  ...
                  <div className="mt-8 space-y-5">
                    {pillar.treatments.map((t) => (
                      <div
                        key={t.name}
                        className="border-b border-[#B0CFEE] pb-5 last:border-0 last:pb-0"
                      >
      ...
      <section className="mx-auto max-w-7xl px-5 sm:px-8 pb-8 pt-4">
        <ScrollReveal>
          <div className="flex flex-col gap-4 rounded-2xl border border-[#B0CFEE] bg-white p-7 sm:flex-row sm:items-center sm:gap-6">
      ...
      <section className="mx-auto max-w-7xl px-5 sm:px-8 py-20">
        <ScrollReveal>
          <div className="rounded-3xl border border-[#B0CFEE] bg-white px-8 py-14 text-center sm:py-16">
